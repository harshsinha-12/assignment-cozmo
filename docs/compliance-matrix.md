...
| R4 | Video tier, handheld walkthrough | `recon/video.py` + `io/video_poses.py` + `recon/video_pose_alignment.py` | Videos preserve frame/time identity and yield scale-free segments; strict optional metric-pose alignment works synthetically, but current MP4s lack sidecars and FloorPlan output | partial |
