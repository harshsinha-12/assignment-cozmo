...
| R8 | Damage regions, class + metric extent | agent + tools (`docs/agent-layer.md`) | `damage[]` | missing |
| R9 | Concealed-damage flags + rule id | agent `fire_concealed_rule` | `concealed_flags[]` | missing |
| R10 | Scope line items keyed to surfaces | agent `add_scope_line` (qty from geometry) | `scope[]` | missing |
| R11 | Confidence interval on every measurement | schema | `error_interval` required | missing |
