...
| R4 | Video tier, handheld walkthrough | `recon/video.py` + `recon/video_tracks.py` + `recon/video_trajectory.py` | Every room video is orientation-normalized, feature-gated, and converted into explicit scale-free pose segments; metric scale/FloorPlan pending | partial |
