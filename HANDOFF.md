...
- Completed **T7b**, multi-walkthrough, orientation-aware video ingest.
- Added immutable per-video metadata/sample records, explicit quarter-turn image utility, per-video sidecar naming, and three focused tests.
- The adapter processes every sorted room video, disables backend auto-rotation, applies container rotation once, and reports identity/duration/display diagnostics.
- Updated the code map, video format/tier docs, compliance row, ADR, roadmap, README, and task queue. No commit was made.
...
- Video jobs ingest every MP4/MOV in stable order, retain filename-stem room identity, normalize quarter-turn display metadata, and associate stem-named pose sidecars. They do **not** emit centimetres yet.
...
- All 83 tests, Ruff, touched-file formatting, compileall, synthetic reproduction, private two-video smoke, and diff checks pass. Video exit 2 is intentional at the non-metric reconstruction boundary.
...
- T7 metric scale/FloorPlan accuracy still needs tracking plus a validated metric pose or known-length source.
...
1. Implement **T7c**: deterministic ORB feature/match/inlier/parallax diagnostics for each normalized room video, rejecting weak/pure-rotation sequences before VO.
2. Then add relative camera tracking and connect it to a validated metric sidecar/scale source, or start T8 SfM against the uploaded photos.
