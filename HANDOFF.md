...
- Completed **T7d**, deterministic scale-free relative camera trajectory recovery.
- Moved reusable ORB/fundamental correspondences into `video_features.py`; the T7c quality gate and T7d pose algorithm now share the same evidence.
- Added assumed-intrinsics essential poses, unit-normalized translation directions, and explicit local segment breaks/restarts after rejected edges.
- Added two-view pose, graph-break, and short-input tests; updated the code map, video docs, compliance row, ADR, roadmap, README, and task queue. No commit was made.
...
- Video jobs ingest every MP4/MOV in stable order, normalize display metadata, associate stem-named sidecars, qualify relative-VO evidence, and recover disconnected unitless pose segments. They do **not** emit centimetres yet.
...
- All 89 tests, Ruff, touched-file formatting, compileall, synthetic reproduction, private two-video trajectory smoke, and diff checks pass. Video exit 2 is intentional at the non-metric reconstruction boundary.
...
- T7 metric scale/FloorPlan accuracy still needs a validated metric pose or known-length source and segment alignment.
...
1. Implement **T7e**: define and validate per-video metric pose-sidecar records, including explicit frame/timestamp correspondence.
2. Align scale-free trajectory segments only where metric correspondence is proven, or start T8 SfM against the uploaded photos.
3. Keep collecting the missing T3 evidence in parallel; do not score accuracy without tape truth.

### Read next (max five)

1. `TASKS.md`
2. `src/cozmo_floorplan/recon/video_trajectory.py`
3. `src/cozmo_floorplan/recon/video.py`
4. `src/cozmo_floorplan/recon/video_features.py`
