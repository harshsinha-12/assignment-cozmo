...
dimensions. Same run: walls 12.5 cm median / 25 cm p95;
...
## 2026-09-10 — T20e submission README and GitHub hygiene



### Done

- Refreshed the main README around the reviewer path: clean setup, one-command
run, generated artifacts, photo/video/LiDAR job layouts, private benchmark,
Route 1 cable installation, Route 2 capture, and defense rehearsal.
- Added an explicit weighted scoring-status table. Both permitted capture routes
are implemented and exercised, and all scoring rows are mapped, without
claiming that the current photo/video or remaining LiDAR gates pass.
- Replaced optimistic capture-quality language with the current measured
limitations from `docs/writeup.md` and `docs/compliance-matrix.md`.
- Audited tracked non-submission files. Raw Route 1 captures, Xcode per-user
state, `START-TOMORROW.md`, `discussion.md`, `mytask.md`, `url.md`, and the
duplicate recruiter PDF with the newline filename are Git-ignored and removed
from the index. Their local copies remain. Canonical evidence, the cleaned
`docs/job-brief.md`, canonical PDF, technical docs, and process history remain.
- README relative-link validation, Ruff, compileall, staged/unstaged
`git diff --check`, and `make reproduce-synthetic` (1.67 s) pass. Full pytest
reports 176 passed and one failure: the existing Route 1 installer dry-run
times out waiting for Apple's `CoreDeviceService`. No installer code changed
...
- Review the README and staged removals, then commit if approved. Code work
remains T8c only after a connected photo graph, plus evidence-driven T6/T7
