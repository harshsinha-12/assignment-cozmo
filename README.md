...
**Schema, CLI, eval, RoomPlan JSON LiDAR, T9 stitch/ablation, multi-video/photo ingest, paired JSON/SVG, T16 claims agent/tools, T19 fix loop, T10 report draft, and one-command synthetic reproduction work. Video now has orientation-aware ingest, ORB/geometric feature gates, and segmented scale-free relative poses; real Record3D emits partial metric geometry. Video scale/FloorPlan output, cross-scan registration, calibration, and metric photos remain.**
...
Video jobs: put one MP4/MOV per room in `video/` (`docs/formats/video-job.md`). The CLI samples every file, applies container display rotation exactly once, evaluates ORB matches/geometric inliers/parallax/coverage, and recovers segmented unitless camera poses with an explicitly unvalidated focal prior. It still exits with a structured failure rather than guessing centimetres.
